...
__global__ void histogram256Kernel(uint *d_PartialHistograms, uint *d_Data, uint dataCount){
    ...
    // pos = blockID * blockDIM + threadID ,  pos < dataCount (input_size), pos += blockDIM*gridDIM
...
__global__ void mergeHistogram256Kernel(
    ...
    uint sum = 0;
    for(uint i = threadIdx.x; i < histogramCount; i += MERGE_THREADBLOCK_SIZE) // Divergence for last warp as only 16 threads are active
